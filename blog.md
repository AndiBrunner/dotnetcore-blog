# First steps with .NET Core in a Cloud Foundry PaaS

<br>
<br>

This post gives you an introduction to .NET Core and how you can deploy a .NET Core application to Cloud Foundry from scratch.

The latest version of .NET Core, 1.1.0, was released November 16th 2016 ([Announcement](https://blogs.msdn.microsoft.com/dotnet/2016/11/16/announcing-net-core-1-1/)) and 
brings some improvements across .NET Core 1.1, ASP.NET Core and EF Core that will make your apps better and faster.
...
As .NET Core now supports Linux it's possible to run apps in a Cloud Foundry Ubuntu container. For that you build your .NET Core app and deploy it with the .NET buildpack to Cloud Foundry. 
...
Starting from scratch you have to:

1. Install .NET Core SDK
2. Install Visual Studio Code
3. Setup an account on a Cloud Foundry PaaS
...
## Setup an account on a Cloud Foundry PaaS
...
Follow the [instructions](https://docs.developer.swisscom.com/cf-cli/install-go-cli.html) to install the CLI.
...
To create a new project open a new console/terminal and follow this steps:

1. Create a new folder/directory
2. Change to the folder
3. Run "dotnet new" to create a new project
4. Run "dotnet restore" to fetch all required NuGet packages linked in the project
5. Run "dotnet run" to execute the app
...
Now we open the "hello" project with Visual Studio Code (or an editor by your choice) and we modify it that way it becomes a web application.
...
Currently, for cross-platform applications we have to use the Kestrel server.
...
We will switch to `1.1.0` later in this post.
...
After we made the change to the `project.json` Visual Studio Code asks for restoring. Confirm the message. 
...
This updates the NuGet packages and pulls down the additional package.
...
Middleware is essentially what we thought of as HttpHandlers and HttpModules in classic ASP.NET.
...
When App.Run() gets fired it’s expected to handle the inbound request completely and return a result.
...
When you build a Web application you tend use much higher abstractions such as ASP.NET MVC.
...
* add an additional reference to the `project.json`:
...
A MVC API application is deployed to Cloud Foundry by using the .NET Core buildpack. You can imagine a buildpack as an installation program.
...
The .NET Core buildpack configures the app web server automatically and you don't have to handle this yourself.
But you have to prepare your app that way the buildpack can deliver this information via the command line to your app.
...
* put an additional reference to project.json (as there is no `1.0.1` version for CommandLine so we use `1.0.0` instead):
    ```c#
    "Microsoft.Extensions.Configuration.CommandLine": "1.0.0",
    ```

Restore it and then run the application on a different port than 5000 (default port):
```shell
dotnet restore
...
Currently, the buildpack has an issue for option 1:  the app is compiled just before starting the app (running container) instead of during the staging phase.
...
Second, the NuGet cache is also stored in your container and uses several hundred megabytes disk space. 
...
For example, when you are using the Swisscom Application Cloud :
...
If everything went well you can call your app with the URL of your app. You can find out your URL with the `cf app` command:
```shell
cf app myApp
```
![app output](app.png)
...
Then open the URL <http://mycfapp.scapp.io/HelloWorld>. E voila!
...
We have also to add a global.json file and add some content. Otherwise the buildpack will use an older .NET CLI to run the app:
